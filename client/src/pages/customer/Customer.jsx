    ...
    <>
      ...
      <div className="bg-gray-100 flex items-center justify-between px-4 py-3 border-b-[1px]">
        ...
        <div className="flex gap-2">
          <Link to={`/customers/form/${id}`} className="btn-outlined" >EDIT</Link>
          <button className="btn-red" onClick={(e) => deleteCustomerInfo(e, id)}>DELETE</button>
        </div>
      </div>
      <div className="px-4 py-8 grid gap-6">
        <div className="grid gap-1 mb-4">
          <span className="text-gray-500 text-sm font-semibold">CUSTOMER CODE</span>
          <span className="text-xl text-blue-400 bg-gray-200 max-w-max px-2 rounded-md">#{customerInfo._id ? customerInfo._id.substring(0,10) : ""}</span>
        </div>
        ...
        <div className="grid grid-cols-[200px_1fr] items-center">
          ...
          <div className="grid grid-cols-[250px_250px]">
            <div className="grid gap-2">
              ...
              <span>{customerInfo.contact && customerInfo.contact.phoneNumber}</span>
            </div>
            <div className="grid gap-2">
              <span className="font-semibold text-gray-500 text-sm">EMAIL ADDRESS</span>
              <span>{customerInfo.contact && customerInfo.contact.email ? customerInfo.contact.email : "N/A"}</span>
            </div>
          </div>
        </div>
      </div>
